            ...
            ) : COLOR
{
    ...
    // w interpolujemy oddzielnia bo inaczej wynik jest do bani - wyglada to na blad kompilatora HLSL...
    half4 src;
    src.xyz=lerp(src1.xyz, src2.xyz, In.halfAngle.w);
    src.w=lerp(src1.w*FloatParam0_SpecularScale, src2.w*FloatParam1_SpecularScale2, In.halfAngle.w);


    half3 lightmap=half3(tex2D(Texture1,In.uv1).xyz);
    half3 bumpNormal=2*(half3(tex2D(Texture3,In.uv0).xyz)-0.5.xxx);
    bumpNormal=normalize(bumpNormal);

    half4 diffuse=half4(saturate(lightmap+In.diffuse.xyz),1);
    
    half specularFactor=saturate(dot(bumpNormal,normalize(In.halfAngle.xyz)));
    ...
    spec*=In.specular*saturate(blendedLM + In.diffuse.xyz);
    spec*=saturate(src.w+FloatParam2_SpecularAdd)*(1-In.halfAngle.w);

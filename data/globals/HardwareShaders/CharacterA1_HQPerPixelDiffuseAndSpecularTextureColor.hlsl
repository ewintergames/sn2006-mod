...
// #default SpecularTextureFactor = 0.5
...
half4 main(in Vert2Frag In, 
            ...
            uniform half FloatParam5_SpecularTextureFactor,
            uniform half4 BestLightsIntensityVector
            ) : COLOR
{
...
//    half3 halfAngle2=normalize(In.light2HalfAngle.xyz);
    ...
    half3 specularTextureColor=textureColor.xyz*(dot(specularFactor,BestLightsIntensityVector.xyz));
    specularColor=lerp(specularColor, specularTextureColor, FloatParam5_SpecularTextureFactor);
    
    half4 color=half4(textureColor.xyz*light + specularColor*textureColor.w,Blend.x);
        ...
        )
{
    ...
    vout.diffuse = float4(color.xyz*FloatParam2_DiffuseMultiplier,1);
    vout.uv=vin.uv0;
	vout.fog=1 - fogFactor(pos,EyeInObjectSpace.xyz,FogParams);
}

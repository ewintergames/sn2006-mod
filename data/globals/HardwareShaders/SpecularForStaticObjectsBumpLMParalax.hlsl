...
// #default HeightFieldScale = 1

half4 main(in Vert2Frag In, 
            uniform sampler2D Texture0, uniform sampler2D Texture1,
            uniform sampler2D Texture2_NormalMap,
			uniform sampler2D Texture3_HeightField,
...
{
	float hf_tex = tex2D(Texture3_HeightField, In.uv0);	
	float hf = FloatParam3_HeightFieldBias + hf_tex * FloatParam4_HeightFieldScale;
	float3 e = normalize(In.eyeVector);
	//float2 tn = In.uv0 + (hf/e.z)*e.xy;
	float2 tmCoords = In.uv0 + hf*e.xy;
	float2 lmCoords = In.uv1 + hf*e.xy;
	

    half4 src=half4(tex2D(Texture0,tmCoords));
    half3 lightmap=half3(tex2D(Texture1, lmCoords).xyz);
    
    half3 bumpNormal=2*(half3(tex2D(Texture2_NormalMap, tmCoords).xyz)-0.5.xxx);

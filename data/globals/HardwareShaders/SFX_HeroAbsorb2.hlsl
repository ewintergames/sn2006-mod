        ...
        )
{
    ...
    float2 uvVec=vin.uv0-float2(0.5,0.7);
    float uvL=length(uvVec);
    
    vout.noise1UV = float2(uvL,vin.uv0.x)*VectorParam1_Noise1.zy+float2(frac(VectorParam1_Noise1.x*FrameTime),0);
    vout.noise2UV = float2(vin.uv0.y,uvL)*VectorParam2_Noise2.zy+float2(0,frac(VectorParam2_Noise2.x*FrameTime));
    
    uvL/=0.7;
    float clampDown = saturate(FloatParam2_EffectClampScale*(uvL+saturate(FloatParam1_EffectPosition)-1));
    float clampUp = saturate(FloatParam2_EffectClampScale*(1-uvL-saturate(FloatParam1_EffectPosition-1)));

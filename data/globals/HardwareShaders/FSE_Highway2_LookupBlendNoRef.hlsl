        ...
        uniform float4x4 ModelViewProjMatrix)
{
    ...
    vout.screenUV = float2(vin.screenUV.x,1-vin.screenUV.y);
            ...
            uniform sampler2D Texture4_Color_lookup, uniform half4 VectorParam3_Lookup_intensity) : COLOR
{
    ...
    picture=picture*VectorParam2_GlowFactor.x+glow*VectorParam2_GlowFactor.y;
...
half4 main1_4(in Vert2Frag1_1 In, uniform sampler2D Texture0, uniform sampler2D Texture1,
            ...
            uniform sampler2D Texture4_Color_lookup, uniform half4 VectorParam3_Lookup_intensity) : COLOR
{
    half3 picture=tex2D(Texture1,In.mainUV);
    half3 glow=half3(tex2D(Texture0,In.glowUV).xyz)*ConstantColor.xyz;
    picture=picture*VectorParam2_GlowFactor.x+glow*VectorParam2_GlowFactor.y;
...
// no dep. tex, only 4tex
half4 main1_1(in Vert2Frag1_1 In, uniform sampler2D Texture0, uniform sampler2D Texture1,
            uniform half4 VectorParam2_GlowFactor,
            uniform half4 ConstantColor ) : COLOR
{
    half3 picture=tex2D(Texture1,In.mainUV);
    half3 glow=half3(tex2D(Texture0,In.glowUV).xyz)*ConstantColor.xyz;
	// without saturate, on nvidia GF5950Ultra there was a black screen
    picture=picture*saturate(VectorParam2_GlowFactor.x)+glow*saturate(VectorParam2_GlowFactor.y);

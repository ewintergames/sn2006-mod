...
half4 main(in Vert2Frag In, uniform sampler2D Texture0, uniform sampler2D Texture1,
            ...
            uniform sampler2D Texture3_Tiled_mask, uniform half4 VectorParam2_Tiled_mask_color,
            ...
            uniform half4 VectorParam3_Lookup_intensity) : COLOR
{
    ...
    half intensity=dot(result2,BW);
    half3 lookup=half3(tex2D(Texture4_Color_lookup,half2(intensity,0.5)).xyz);
    return half4(lookup*VectorParam3_Lookup_intensity.x+result2*VectorParam3_Lookup_intensity.y,1);
}
